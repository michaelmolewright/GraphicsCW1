...
ParticleField::ParticleField( RNG& aRNG, std::size_t aImageWidth, std::size_t aImageHeight, ColorF const& aParticleColor, float aParticleDensity, float aParticleSpeedMult, float aPadding )
...
void ParticleField::draw( Surface& aSurface ) const
{
	for( auto const& particle : mParticles )
	{
		...
		std::size_t const xpos = std::size_t( p.x + .5f );
		std::size_t const ypos = std::size_t( p.y + .5f );
...
void ParticleField::resize( std::size_t aImageWidth, std::size_t aImageHeight )
{
